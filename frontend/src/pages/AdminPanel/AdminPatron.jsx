...
const AdminPatron = () => {
  ...
  // fetch data
  const [PatronTeamData, setPatronTeamData] = useState([]);
  ...
  const handleUpload = async (e) => {
    ...
    const { file, name, designation, phone, email } = formData;
    ...
    <>
      ...
      <div className="flex justify-center items-center min-h-screen w-[100%] bg-gray-100">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mx-auto p-4">
          {PatronTeamData.length > 0 ? (
            PatronTeamData.map((data, index) => (
              <div
                key={data.id || index}
