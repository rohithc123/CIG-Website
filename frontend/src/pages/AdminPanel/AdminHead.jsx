...
const AdminHead = () => {
  ...
  // fetch data
  const [HeadTeamData, setHeadTeamData] = useState([]);
  ...
  const deleteUser = async (id) => {
    try {
      const response = await fetch(`http://localhost:3000/api/admin/teamMembersUploads/delete/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: AuthorizationToken,
        },
  ...
  const handleUpload = async (e) => {
    ...
    const { file, name, designation, phone, email,Linkedin } = formData;
    ...
    try {
      const response = await axios.post(
        'http://localhost:3000/api/admin/teamMembersUploads',
        uploadData,
        {
          headers: {
    ...
    <>
      <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
        ...
        <form onSubmit={handleUpload} className="bg-white p-6 rounded shadow-md w-full max-w-md">
          {['name', 'designation', 'phone', 'email','Linkedin'].map((field) => (
            <input
              key={field}
              type="text"
              name={field}
      ...
      <div className="flex justify-center items-center min-h-screen w-[100%] bg-gray-100">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mx-auto p-4">
          {HeadTeamData.length > 0 ? (
            HeadTeamData.map((data, index) => (
              <div
                key={data.id || index}
              ...
              >
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <div className="text-[#ABABAB] font-medium text-base">LinkedIn: {data.Linkedin}</div>
                  <div className="text-[#ABABAB] font-medium text-base">{data.email}</div>
