...
const AdminBlog = () => {
    ...
    const deleteUser = async (id) => {
      try {
        const response = await fetch(`http://localhost:3000/api/admin/BlogUploads/delete/${id}`, {
          method: 'DELETE',
          headers: {
            Authorization: AuthorizationToken,
          },
    ...
    const handleUpload = async (e) => {
      ...
      const { file, title,name, description } = formData;
      ...
      try {
        const response = await axios.post(
          'http://localhost:3000/api/admin/BlogUploads',
          uploadData,
          {
            headers: {
      ...
      <>
        <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
          ...
          <form onSubmit={handleUpload} className="bg-white p-6 rounded shadow-md w-full max-w-md">
            {['title', 'name', 'description'].map((field) => (
              <input
                key={field}
                type="text"
                name={field}
              ...
              BlogData.map((data, index) => (
                ...
                >
                  ...
                  <div className="flex flex-col justify-center">
                    ...
                    <div className="text-[#183059] font-medium text-base">{data.name}</div>
